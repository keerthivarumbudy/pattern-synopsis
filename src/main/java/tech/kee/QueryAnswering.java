...
public class QueryAnswering {
    ...
    public static void upperBoundExperiments(List<Integer> windows, StreamSummary streamSummary) throws IOException {
        assert windows.stream().allMatch(window -> window >= streamSummary.resolutionSeconds)
                : "All windows should be greater than or equal to resolution";
        windows = windows.stream()
                .map(window -> window / streamSummary.resolutionSeconds)
                .toList();
        // choose the smallest prime number to be the smallest window size for the lowest composed block
        List<Integer> blockWindows = Utils.primeFactorization(
                windows.get(0)); // this can be modified to be the smallest prime factor of all windows
        ...
        for (List<Integer> pattern : patternMapsPerLayer.keySet()) {
            ...
            for (int i = 0; i < temporaryStreamSummary.layerSketchList.size() - 1; i++) {
                if (patternMapsPerLayer.get(pattern).get(i)
                        > patternMapsPerLayer.get(pattern).get(i + 1))
                    //                    System.out.println("Upperbound of layer with resolution="+
                    // temporaryStreamSummary.layerSketchList.get(i).get(0).resolution+" is less than layer with
                    // resolution="+ temporaryStreamSummary.layerSketchList.get(i+1).get(0).resolution);
